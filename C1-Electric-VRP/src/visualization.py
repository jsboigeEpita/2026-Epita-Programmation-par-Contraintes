"""Visualisation utilities for the EVRP."""
...
def plot_instance(instance: EVRPInstance, ax: Optional[plt.Axes] = None, title: str = "") -> plt.Axes:
    """Plot depot, customers and charging stations."""
    if ax is None:
    ...
    for i in inst.customer_indices:
        ax.annotate(str(i), (xs[i], ys[i]), textcoords="offset points",
                    xytext=(5, 5), fontsize=7, color="steelblue")

    # Charging stations
    if inst.station_indices:
        sx = [xs[i] for i in inst.station_indices]
        sy = [ys[i] for i in inst.station_indices]
        ax.scatter(sx, sy, c="green", s=200, marker="^", zorder=4,
                   label=f"Stations ({inst.n_stations})")
        for i in inst.station_indices:
            ax.annotate(f"S{i}", (xs[i], ys[i]), textcoords="offset points",
...
def plot_routes(
    ...
    """Plot routes on top of the instance."""
    ...
    for k, route in enumerate(routes):
        ...
        legend_patches.append(
            mpatches.Patch(color=color,
                           label=f"V{k}: {len([n for n in route if inst.node_types[n]==CUSTOMER])} cust, load={load}/{inst.vehicle_capacity}")
        )
...
def plot_battery_profile(
    ...
    """Plot battery state-of-charge along a single vehicle route."""
    if ax is None:
    ...
    for idx in range(len(full_path) - 1):
        ...
        if inst.node_types[i] == STATION:
            battery = inst.battery_capacity  # full recharge
    ...
    ax.axhline(inst.battery_capacity / ENERGY_SCALE, color="green",
               linestyle="--", label="Full battery")
...
def plot_comparison(
    ...
    """Bar chart comparing methods by distance and solve time."""
