"""Cache disque SHA-256 pour les appels LLM.

Identique en principe au cache de I3-Arthur-Amaury/cp_llm/cache.py.
"""
...
class CachedLLMClient(LLMClient):
    """Enveloppe un LLMClient et persiste les reponses sur disque."""
    ...
    def call_structured(self, system_prompt: str, user_prompt: str, output_schema: type[T]) -> T:
        key = _digest(self._provider, str(self._model), output_schema.__name__,
    ...
    def call_text(self, system_prompt: str, user_prompt: str, temperature: float = 0.0) -> str:
        parts = [self._provider, str(self._model), "text", system_prompt, user_prompt]
        if temperature != 0.0:
